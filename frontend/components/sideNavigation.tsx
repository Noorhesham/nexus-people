...
import { AccountBalance, Architecture, Handshake, Home, PhoneAndroid } from '@mui/icons-material';
...
const sections = [
  ...
  {section: 'Units', title: 'section.smart'},
  {section: 'services', title: 'project.plan'},
  {section: 'brands', title: 'section.partners'},
];
                ...
                >
                  <Tooltip title={<FormattedMessage id={section.title}/>} arrow placement='left'>
                    <motion.div className={`${getIconScale(section.section)} hover:scale-150 transiton ease duration-300`}>
                      {section.section === 'home' && <Home />}
                      {section.section === 'Units' && <PhoneAndroid />}
                      {section.section === 'project' && <Architecture />}
                      {section.section === 'services' && <AccountBalance />}
                      {section.section === 'brands' && <Handshake />}
                    </motion.div>
                  </Tooltip>
                </Link>
              ))}
