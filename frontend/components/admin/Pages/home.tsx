...
const Home = () => {
  ...
  useEffect(() => {
    fetchData(apiEndpoint, "Data", Cookies.get('token'))
  ...
  useEffect(() => {
    ...
    const imagesArray = galleryArray?.map((gallery:any) => gallery.images)
      .flat();
      imagesArray?.forEach((image: any) => {
        image.Type = 'image'
      })

    const videosArray = galleryArray?.map((gallery: any) => gallery.videos)
      .flat();
  ...
  const columns : any = [
    ...
    renderCell: ({ row }: Partial<GridRowParams>) =>
      <IconButton className="hover:text-red-500" onClick={() => 
        {handleDelete(row._id, `${process.env.BACKEND}home/${row.Type}`, Cookies.get('token'))
    ...
    <div className="relative w-full  h-full overflow-y-auto">
      ...
      <NewModal open={isOpen} onClose={ModalHandler}>
        <div>
          ...
          {selectedTab === 'Image' ? <HomeContent type="image" closeModal={ModalHandler}/> : null}
          {selectedTab === 'Video' ? <HomeContent type="video" closeModal={ModalHandler}/> : null}
