...
const Pages = () => {
  ...
  useEffect(() => {
    fetchData(apiEndpoint, "Background Images", Cookies.get('token'))
  ...
  const columns : any = [
    { field: 'Page', headerName: 'Page', width: 150, resizable: true, valueGetter: (params: any) => params.row?.page || '' },
    ...
    renderCell: ({ row }: Partial<GridRowParams>) =>
      <IconButton className="hover:text-red-500" onClick={() => 
        {handleDelete(row._id, `${process.env.BACKEND}background`, Cookies.get('token'))
    ...
    <div className="relative w-full  h-full overflow-y-auto">
      <div className="h-full">
        ...
        <div className="h-[72vh]">
          <DataTable
            ...
            columns={columns}
            isLoading={isLoading}
            api={`${process.env.BACKEND}background`}
            type="Image"
          />
        </div>
      </div>
      ...
      <NewModal open={isOpen} onClose={ModalHandler}>
        <div>
          ...
          {selectedTab === 'Image' ? <PageContent type="image" closeModal={ModalHandler}/> : null}
          {selectedTab === 'Video' ? <PageContent type="video" closeModal={ModalHandler}/> : null}
