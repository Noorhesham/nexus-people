...
const activateAccount = catchError(async (req, res, next) => {
  ...
  jwt.verify(token, process.env.EMAIL_SIGNATURE, async (err, decoded) => {
    if (err) {
      ...
      if (user.verified) {
        return res.redirect(`${process.env.FRONTEND}sign`);
      }
      ...
      if (user.matchedCount) {
        ...
        // return res.status(404).redirect(`${process.env.FRONTEND}sign`_REGISTER_URL); //Signup page
        return next(
          new AppError(
...
const newConfirmEmail = catchError(async (req, res, next) => {
  ...
  jwt.verify(token, process.env.EMAIL_SIGNATURE, async (err, decoded) => {
    if (err) {
      ...
      let link = `https://nexusbhub.com/backend/auth/confirmEmail/${newToken}`;
      let refreshLink = `https://nexusbhub.com/backend/auth/newConfirmEmail/${token}`;
...
const forgetPassword = catchError(async (req, res, next) => {
  ...
  const resetPasswordLink = `https://nexusbhub.com/sign/reset/${token}`;
...
passport.use(
  new GoogleStrategy(
    {
      clientID:
        "560776890096-q3hb7esssda554frb534jnbcq4prp2r9.apps.googleusercontent.com",
      clientSecret: "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",
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        const generatedPassword = customAlphabet(
          ...
          8
        ).toString(); // Implement this function to generate a secure random password
